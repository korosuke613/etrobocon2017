...
* 
* 【コンパイル・実行の仕方】
* g++-7 -w ../src/Pid.cpp ../src/SpeedControl.cpp -I../include
* ./a.out
*/
...
void calcVirtualWheels(){
    ...
    std::mt19937 mt(rnd());
    std::uniform_int_distribution<int> rand10(0,50); // 1~10の一様乱数
    //std::cout << rand10(mt) << std::endl;
    double r_value = rand10(mt) * 0.1;

    // 実際に進んだ距離とする
    left_motor = right_motor += (speed100ms/25 + value + r_value) * 0.1; //+ r_value;
}